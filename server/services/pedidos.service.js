...
export const crearPedido = async ({ id_mesa, tipo, userId, items }) => {
  if (tipo !== "Salon" && tipo !== "Llevar") {
    throw Object.assign(new Error("Tipo de pedido invalido"), { status: 400 });
  }
    ...
    if (items && items.length > 0) {
      for (const item of items) {
        ...
        if (platillo.length > 0) {
          if (
            platillo[0].platillo_disponible === 0 ||
            platillo[0].platillo_disponible === false
          ) {
            throw Object.assign(
              new Error("No se puede agregar un platillo no disponible"),
              { status: 400 },
            );
          }
...
export const agregarItemsPedido = async ({ id_pedido, items }) => {
  if (!id_pedido) {
    throw Object.assign(new Error("Debe enviar el id del pedido"), {
      status: 400,
    });
  }
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (platillo.length === 0) {
        throw Object.assign(new Error("Platillo no encontrado"), {
          status: 404,
        });
      }
...
export const iniciarPedido = async ({ id_mesa, tipo, userId, items }) => {
  ...
  if (tipo === "Salon") {
    ...
    if (mesa[0].mesa_estado !== "Disponible") {
      throw Object.assign(new Error("La mesa no esta disponible"), {
        status: 400,
      });
    }
  }
...
export const agregarPlatilloAPedido = async ({
  ...
  if (detalle_pedido_notas) {
    if (typeof detalle_pedido_notas !== "string") {
      throw Object.assign(new Error("Las notas deben ser texto"), {
        status: 400,
      });
    }
  ...
  if (
    ...
    throw Object.assign(new Error("El platillo no está disponible"), {
      status: 400,
    });
  }
...
export const eliminarPlatilloPedido = async (id_detalle) => {
  ...
  if (detalleRows.length === 0) {
    throw Object.assign(new Error("Detalle de pedido no encontrado"), {
      status: 404,
    });
  }
...
export const enviarPedidoACocina = async (id_pedido, userId) => {
  ...
  if (detalle[0].total === 0) {
    throw Object.assign(new Error("No puedes enviar un pedido sin platillos"), {
      status: 400,
    });
  }
...
`,
  ...
  if (result.affectedRows === 0) {
    throw Object.assign(new Error("El pedido ya no está en estado 'Listo'"), {
      status: 400,
    });
  }
  ...
  rows.forEach((row) => {
    if (!pedidosMap[row.id_pedido]) {
      pedidosMap[row.id_pedido] = {
        ...
        mesa: row.pedido_tipo === "Llevar" ? "Para llevar" : row.mesa_numero,
...
export const cancelarPedido = async (id_pedido, motivo, userId) => {
  ...
  if (pedido.pedido_estado === "Cerrado") {
    throw Object.assign(new Error("No se puede cancelar un pedido facturado"), {
      status: 400,
    });
  }
  ...
  if (pedido.pedido_estado === "Cancelado") {
    throw Object.assign(new Error("El pedido ya fue cancelado anteriormente"), {
      status: 400,
    });
  }
  ...
  await db.query(
    `UPDATE pedidos
     ...
     WHERE id_pedido = ?`,
    [motivo || null, userId, estadoAnterior, id_pedido],
  );
...
export const cambiarEstadoPedidoCocina = async (id_pedido, nuevoEstado) => {
  ...
  if (nuevoEstado === "Preparado" && pedido.pedido_estado !== "EnPreparacion") {
    throw Object.assign(
